...
/**
 * Creates an empty game board of maximum possible size.
...
export const recalculateBoardStatuses = (gameState: GameState): Board => {
  ...
  for (let r = 0; r < GRID_SIZE; r++) {
    for (let c = 0; c < GRID_SIZE; c++) {
      ...
      for (const pos of neighborsPos) {
        ...
        if (nr >= 0 && nr < GRID_SIZE && nc >= 0 && nc < GRID_SIZE) {
          ...
          if (neighborCard?.ownerId !== undefined && !neighborCard.isFaceDown && !isNeighborStunned) {
            ...
            // A neighbor is friendly if they are same player, or if they are on same team (and teams exist).
      ...
      // Apply "Threat" Status Condition A: Pinned by two cards of same enemy.
      ...
      // Apply "Threat" Status Condition B: On active border with an enemy neighbor.
  ...
  for (const hero of mrPearls) {
    ...
    if (!processedRowsForPearl.has(rowKey)) {
      ...
      for (let i = 0; i < GRID_SIZE; i++) {
        const target = newBoard[r][i].card
        // Exclude hero card itself - only buff OTHER units
